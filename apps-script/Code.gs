...
function diagnose() {
  ...
  if (responses && engine) {
    ...
    out.push('ℹ️ Algorithm Engine!A1 formula: ' + (engine.getRange('A1').getFormula() || '(none)'));
    if (row >= 2) {
      const hdr = responses.getRange(1, 1, 1, 6).getDisplayValues()[0];
      const val = responses.getRange(row, 1, 1, 6).getDisplayValues()[0];
      out.push('ℹ️ "' + responses.getName() + '" row ' + row + ': ' +
               hdr.map((h, i) => String.fromCharCode(65 + i) + ' [' + h + '] = "' + val[i] + '"').join(' | '));
