# Google Sheets Setup & Formulas — European locale
...
> **European formula style.** Same content as [`formulas.md`](formulas.md), converted for
> spreadsheets whose locale uses `;` between arguments (File ▸ Settings ▸ Locale, e.g. Germany,
> France, Netherlands): `;` separates arguments, `\` separates columns in `{…}` arrays, and
> decimals use a comma (`0,9`). Text inside quotes is unchanged. Function names stay in English.
